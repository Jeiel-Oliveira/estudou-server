...
public class GoalControlllerTests {
  ...
  void shouldCreate() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/goal").contentType(MediaType.APPLICATION_JSON).content(goalRequestString))
        .andExpect(status().isCreated());
  }

  @Test
  void shouldFailWithoutRequiredFields() throws Exception {
    mockMvc.perform(post("/api/goal").contentType(MediaType.APPLICATION_JSON).content("{}"))
  ...
  void shouldUpdate() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/goal/1").contentType(MediaType.APPLICATION_JSON).content(goalRequestString))
  ...
  void shouldFindById() throws Exception {
    ...
    mockMvc.perform(get("/api/goal/1").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk());

    MvcResult result = mockMvc.perform(get("/api/goal/1").contentType(MediaType.APPLICATION_JSON))
        .andReturn();
  ...
  void shouldFindAll() throws Exception {
    ...
    mockMvc.perform(get("/api/goal")).andExpect(status().isOk());
  }
  ...
  void shouldProperlyDelete() throws Exception {
    ...
    mockMvc.perform(delete("/api/goal/1", 1L)).andExpect(status().isOk());
  }
