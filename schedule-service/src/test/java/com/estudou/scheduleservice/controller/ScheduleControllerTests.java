...
public class ScheduleControllerTests {
  ...
  void shouldCreate() throws Exception {
    ...
    mockMvc.perform(post("/api/schedule").contentType(MediaType.APPLICATION_JSON)
        .content(scheduleRequestToString)).andExpect(status().isCreated());
  }

  @Test
  void shouldFailCreateWithoutBody() throws Exception {
    mockMvc.perform(post("/api/schedule").contentType(MediaType.APPLICATION_JSON).content("{}"))
  ...
  void shouldUpdate() throws Exception {
    ...
    mockMvc.perform(patch("/api/schedule/1").contentType(MediaType.APPLICATION_JSON)
        .content(scheduleRequestString)).andExpect(status().isOk());
  }
  ...
  void shouldFindById() throws Exception {
    ...
    mockMvc.perform(get("/api/schedule/1", 1L)).andExpect(status().isOk());
  }
  ...
  void shouldProperlyDelete() throws Exception {
    ...
    mockMvc.perform(delete("/api/schedule/1", 1L)).andExpect(status().isOk());
  }
