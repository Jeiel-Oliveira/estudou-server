...
class CategoryControllerTests {
  ...
  void shouldCreate() throws Exception {
    ...
    mockMvc.perform(post("/api/category").contentType(MediaType.APPLICATION_JSON)
        .content(categoryRequestToString)).andExpect(status().isCreated());
  }

  @Test
  void shouldFailCreationWithoutName() throws Exception {
    mockMvc.perform(post("/api/category").contentType(MediaType.APPLICATION_JSON).content("{}"))
  ...
  void shouldUpdate() throws Exception {
    ...
    mockMvc.perform(patch("/api/category/1").contentType(MediaType.APPLICATION_JSON)
        .content(categoryRequestString)).andExpect(status().isOk());
  }
  ...
  void shouldFindById() throws Exception {
    ...
    mockMvc.perform(get("/api/category/1", 1L)).andExpect(status().isOk());
  }
  ...
  void shouldProperlyDelete() throws Exception {
    ...
    mockMvc.perform(delete("/api/category/{categoryId}", 1L)).andExpect(status().isOk());
  }
